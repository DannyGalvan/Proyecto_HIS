...
import { validateAppointmentStatus } from "../../validations/appointmentStatusValidation";
...
interface AppointmentStatusFormProps {
  ...
  readonly initialForm: AppointmentStatusRequest;
  readonly onSubmit: (form: AppointmentStatusRequest) => Promise<ApiResponse<unknown | ValidationFailure[]>>;
}

export function AppointmentStatusForm({ type, initialForm, onSubmit }: AppointmentStatusFormProps) {
  const isEditing = type === "edit";
  const navigate = useNavigate();

  const { form, errors, handleChange, handleSubmit, success, message, loading } =
    useForm<AppointmentStatusRequest, unknown>(initialForm, validateAppointmentStatus, onSubmit, true);
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-center mb-6">
        {isEditing ? "Editar Estado de Cita" : "Crear Estado de Cita"}
      </h1>
      ...
      <Form className="flex flex-col gap-4" validationErrors={errors} onSubmit={handleSubmit}>
        <TextField isRequired className="w-full flex flex-col gap-1" isInvalid={!!errors?.name} name="name" onChange={handleTextChange("name")}>
          <Label className="font-bold">Nombre del Estado</Label>
        ...
        <div className="flex gap-4 justify-end mt-4">
          <AsyncButton className="font-bold" isLoading={false} size="lg" type="button" variant="secondary" onClick={() => navigate("/appointment-status")}>
            Cancelar
          </AsyncButton>
