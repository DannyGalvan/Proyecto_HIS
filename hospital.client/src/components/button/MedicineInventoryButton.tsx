...
export function MedicineInventoryButton({
  ...
  const handleEdit = useCallback(
    () => navigate(`/medicine-inventory/update/${data.id}`),
    [navigate, data.id],
  );
  ...
  const handleDeactivateConfirm = useCallback(async () => {
    ...
    try {
      const response = await partialUpdateMedicineInventory({
        id: data.id,
        state: data.state === 1 ? 0 : 1,
      });
      if (response.success) {
        toast.success(
          `Inventario ${data.state === 1 ? "desactivado" : "activado"} correctamente`,
        );
