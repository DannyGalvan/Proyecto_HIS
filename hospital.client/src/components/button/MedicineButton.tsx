...
interface MedicineButtonProps {
  readonly data: MedicineResponse;
}

export function MedicineButton({ data }: MedicineButtonProps) {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);

  const handleEdit = useCallback(() => navigate(`/medicine/update/${data.id}`), [navigate, data.id]);
  const handleDeleteClick = useCallback(() => setIsDeleteDialogOpen(true), []);

  const handleDeleteConfirm = useCallback(async () => {
    setIsDeleting(true);
    try {
      const response = await deleteMedicine(data.id);
      if (response.success) {
        toast.success(`Medicamento "${data.name}" eliminado correctamente`);
  ...
  return (
    <>
      ...
      <ConfirmDialog cancelText="Cancelar" confirmText="Eliminar" isLoading={isDeleting} isOpen={isDeleteDialogOpen}
        message={`¿Está seguro que desea eliminar el medicamento "${data.name}"?`} title="Confirmar eliminación"
