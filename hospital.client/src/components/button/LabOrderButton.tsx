...
import { deleteLabOrder } from "../../services/labOrderService";
...
export function LabOrderButton({ data }: LabOrderButtonProps) {
  ...
  const handleEdit = useCallback(() => navigate(`/lab-order/update/${data.id}`), [navigate, data.id]);
  const handleDeleteClick = useCallback(() => setIsDeleteDialogOpen(true), []);

  const handleDeleteConfirm = useCallback(async () => {
    setIsDeleting(true);
    try {
      const response = await deleteLabOrder(data.id);
      if (response.success) {
        toast.success(`Orden #${data.orderNumber} eliminada correctamente`);
  ...
  return (
    <>
      ...
      <ConfirmDialog cancelText="Cancelar" confirmText="Eliminar" isLoading={isDeleting} isOpen={isDeleteDialogOpen}
        message={`¿Está seguro que desea eliminar la orden #${data.orderNumber}?`} title="Confirmar eliminación"
