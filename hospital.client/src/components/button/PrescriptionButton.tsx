...
interface PrescriptionButtonProps {
  readonly data: PrescriptionResponse;
}

export function PrescriptionButton({ data }: PrescriptionButtonProps) {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);

  const handleView = useCallback(() => navigate(`/prescription/${data.id}`), [navigate, data.id]);
  const handleDeleteClick = useCallback(() => setIsDeleteDialogOpen(true), []);

  const handleDeleteConfirm = useCallback(async () => {
    setIsDeleting(true);
    try {
      const response = await deletePrescription(data.id);
      if (response.success) {
        toast.success(`Receta #${data.id} eliminada correctamente`);
  ...
  return (
    <>
      <Dropdown>
        ...
        <Dropdown.Popover>
          <Dropdown.Menu aria-label="Acciones">
            <Dropdown.Item key="view" className="text-primary hover:text-white" onClick={handleView}>Ver Detalle</Dropdown.Item>
      ...
      <ConfirmDialog cancelText="Cancelar" confirmText="Eliminar" isLoading={isDeleting} isOpen={isDeleteDialogOpen}
        message={`¿Está seguro que desea eliminar la receta #${data.id}?`} title="Confirmar eliminación"
