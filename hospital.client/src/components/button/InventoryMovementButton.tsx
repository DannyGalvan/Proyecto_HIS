...
export function InventoryMovementButton({
  data,
}: InventoryMovementButtonProps) {
  ...
  const handleView = useCallback(
    () => navigate(`/inventory-movement/${data.id}`),
    [navigate, data.id],
  );
  ...
  const handleDeactivateConfirm = useCallback(async () => {
    ...
    try {
      const response = await partialUpdateInventoryMovement({
        id: data.id,
        state: data.state === 1 ? 0 : 1,
      });
      if (response.success) {
        toast.success(
          `Movimiento ${data.state === 1 ? "desactivado" : "activado"} correctamente`,
        );
        ...
        toast.danger(
          `Error: ${response.message} ${validationFailureToString(response.data)}`,
        );
      }
    } catch (error) {
      toast.danger(
