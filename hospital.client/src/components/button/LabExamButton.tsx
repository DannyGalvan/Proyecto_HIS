import { Button, Dropdown, toast } from "@heroui/react";
import { useQueryClient } from "@tanstack/react-query";
import { useCallback, useState } from "react";
import { useNavigate } from "react-router";
import { deleteLabExam } from "../../services/labExamService";
import type { LabExamResponse } from "../../types/LabExamResponse";
import { validationFailureToString } from "../../utils/converted";
import { Icon } from "../icons/Icon";
import { ConfirmDialog } from "../modal/ConfirmDialog";

import { usePermissions } from "../../hooks/usePermissions";
interface LabExamButtonProps {
  readonly data: LabExamResponse;
}

export function LabExamButton({ data }: LabExamButtonProps) {
  const { can } = usePermissions();
  const canEdit = can("lab-exam/update");
  const canDelete = can("lab-exam/delete");

  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);

  const handleEdit = useCallback(
    () => navigate(`/lab-exam/update/${data.id}`),
    [navigate, data.id],
  );
  const handleDeleteClick = useCallback(() => setIsDeleteDialogOpen(true), []);

  const handleDeleteConfirm = useCallback(async () => {
    setIsDeleting(true);
    try {
      const response = await deleteLabExam(data.id);
      if (response.success) {
        toast.success(
          `El registro ${data.name} ha sido eliminado correctamente.`,
        );
        await queryClient.invalidateQueries({ queryKey: ["lab-exams"] });
        setIsDeleteDialogOpen(false);
      } else {
        toast.danger(
          `No se pudo eliminar: ${response.message} ${validationFailureToString(response.data)}`,
        );
      }
    } catch (error) {
      toast.danger(
        `Error: ${error instanceof Error ? error.message : "Error desconocido"}`,
      );
    } finally {
      setIsDeleting(false);
    }
  }, [data, queryClient]);

  const handleCloseDialog = useCallback(() => {
    if (!isDeleting) setIsDeleteDialogOpen(false);
  }, [isDeleting]);

  if (!canEdit && !canDelete) return null;


  return (
    <>
      <Dropdown>
        <Dropdown.Trigger>
          <Button isIconOnly size="sm" variant="primary">
            <Icon name="bi bi-three-dots-vertical" />
          </Button>
        </Dropdown.Trigger>
        <Dropdown.Popover>
          <Dropdown.Menu aria-label="Acciones">
            {canEdit && (<Dropdown.Item
              key="edit"
              className="text-warning hover:text-white"
              onClick={handleEdit}
            >
              Editar
            </Dropdown.Item>)}
            {canDelete && (<Dropdown.Item
              key="delete"
              className="text-danger hover:text-white"
              onClick={handleDeleteClick}
            >
              Eliminar
            </Dropdown.Item>)}
          </Dropdown.Menu>
        </Dropdown.Popover>
      </Dropdown>
      <ConfirmDialog
        cancelText="Cancelar"
        confirmText="Eliminar"
        isLoading={isDeleting}
        isOpen={isDeleteDialogOpen}
        message={`¿Está seguro que desea eliminar el registro ${data.name}? Esta acción no se puede deshacer.`}
        title="Confirmar eliminación"
        onClose={handleCloseDialog}
        onConfirm={handleDeleteConfirm}
      />
    </>
  );
}
