...
import { deleteVitalSign } from "../../services/vitalSignService";
...
export function VitalSignButton({ data }: VitalSignButtonProps) {
  ...
  const handleEdit = useCallback(() => navigate(`/vital-sign/update/${data.id}`), [navigate, data.id]);
  const handleDeleteClick = useCallback(() => setIsDeleteDialogOpen(true), []);

  const handleDeleteConfirm = useCallback(async () => {
    setIsDeleting(true);
    try {
      const response = await deleteVitalSign(data.id);
      if (response.success) {
        toast.success(`Signos vitales #${data.id} eliminados correctamente`);
  ...
  return (
    <>
      ...
      <ConfirmDialog cancelText="Cancelar" confirmText="Eliminar" isLoading={isDeleting} isOpen={isDeleteDialogOpen}
        message={`¿Está seguro que desea eliminar los signos vitales #${data.id}?`} title="Confirmar eliminación"
