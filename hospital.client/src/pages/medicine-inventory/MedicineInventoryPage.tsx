...
export function MedicineInventoryPage() {
  ...
  const { filters: movementFilters, setFilters: setMovementFilters } =
    useInventoryMovementStore();
  ...
  return (
    <div>
      ...
      <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800 text-sm">
        ⚠️ Los registros marcados en{" "}
        <strong className="text-red-600">rojo</strong> han alcanzado o están por
        debajo del stock mínimo y requieren reorden.
      </div>
      <TableServer
        hasFilters
